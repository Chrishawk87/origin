...
RUN pip install --no-cache-dir fastapi uvicorn python-multipart anthropic openai rich PyYAML requests python-dotenv youtube-transcript-api xhtml2pdf mammoth pymupdf python-docx "sqlalchemy>=2.0" "psycopg[binary]>=3.1"
COPY . .
